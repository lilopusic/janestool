<template>
  <n-h3 class="text-center font-bold">Excel合并工具</n-h3>

  <n-tooltip placement="top-start" trigger="hover">
    <template #trigger>
      <n-input-number placeholder="请输入数字..." v-model:value="startRow" clearable />
    </template>
    从第几行开始合并,默认为三
  </n-tooltip>
  ...
  <n-tooltip placement="top-start" trigger="hover">
    <template #trigger>
      <n-input-number placeholder="请输入数字..." v-model:value="maxCol" clearable />
    </template>
    合并最大列数,默认为50
  </n-tooltip>
  ...
  <n-divider></n-divider>
  <n-button-group>
    <n-button :loading="merging" @click="merge">合并</n-button>
    <n-button @click="purge">清空</n-button>
  </n-button-group>
</template>
<script setup>
import { ref } from 'vue';
